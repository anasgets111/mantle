...
| `just stubs` | Regenerate `lua-meta/mantle.lua` and `docs/capabilities/<name>.md` |
...
| `just hooks` | Once per clone: pre-commit runs the relevant half of `check` on the staged tree and refuses a stale `mantle.lua` or capability page |
