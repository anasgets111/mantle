...
| `docs/{nodes,surfaces}/*.md`, `docs/guide/paint.md` | The property table between the `Generated from ...properties.rs` markers is `just stubs` output; edit the row in `renderer/src/lua/nodes/properties.rs`. The rest of the page is hand-written |
...
| `nodes.lua`, `surfaces.lua` | `PROPERTIES` in `renderer/src/lua/nodes/properties.rs`, which the name check and the shared parsers also read; aliases in `nodes/stubs.rs` | `renderer/src/lua/nodes/stubs.rs`; `every_type_the_stubs_declare_is_accepted_by_the_engine` probes each type |
