...
--
-- `mantle init` rewrites installed stubs that differ from its own copy. Stubs describing another
...
---what types the callback: the `n` in `mantle.network:map(function(n) ... end)` is a `NetworkState`,
...
--
-- `mantle.idle` used to be here too. It joined the roster with ADR-0141, because there turned out
...
---@field minor integer The crate's minor version. Nothing bumps it for an IDL change, so `mantle.version.minor >= n` gates on releases, not fields.
---@field patch integer Everything else. Never affects what a config may use.
"#;

const MANTLE_TAIL: &str = r#"---@field screens ReadOnlyCapability<Screen[]> Renderer-sourced, seeded to an empty list, and the one signal with a value at first evaluation (ADR-0041).
