...
| **IDL** | The typed engine contract in `lua-meta/`, generated from Rust: capability payloads and actions from their types; node and surface properties from the typed fields the parsers read, their Rust types and `///` docs; globals from their Rust signatures and `///` docs; nested table shapes from the structs their parsers fill. Still hand-written: the other aliases in the `nodes.lua` header (`Gradient`, `GradientStop`, `Mask`, `Cursor`, `Easing`, `Animation`, `Animations`, `Exit`; `Align`, `Percent`, `EasingName` and `PopupAnchor` are filled from Rust) and the `StateSignal`, `ScrollSignal`, `PersistentTable` and `SessionProcessHandle` classes, which LuaLS models with generics or dynamic keys. |
