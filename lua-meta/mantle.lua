...
---@field mac string MAC address from WirePlumber's `bluez_card.` name, spelled as `mantle.bluetooth` spells it.
...
---@field name string Connector name, e.g. `"eDP-1"`; matches `mantle.screens.name` and a surface's `monitor`.
...
---@field started_at? integer Unix seconds when the current or last run began; `nil` until the first `start`. Elapsed time is this subtracted from `mantle.system`'s clock, so nothing here needs a second timer.
...
---@field on_battery? boolean Running on battery rather than mains, from UPower; `nil` without UPower. This is the mains question; charge direction is `mantle.battery.state`.
...
---@field microphone_users PrivacyUser[] Apps PipeWire reports reading a microphone now (ADR-0137). Open-but-idle streams are absent: this means "something is listening", not "something could". Distinct from `mantle.audio.source_muted`, a device setting; a muted active capture appears in both.
...
---@field reboot_required boolean Whether `/run/mantle-reboot-required` exists. A pacman hook writes it, so a terminal upgrade raises it too, and `/run` being tmpfs means a boot clears it. Nothing in Mantle writes or clears it.
