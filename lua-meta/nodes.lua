...
---@field on_navigate? fun(key: "up"|"down"|"page_up"|"page_down"|"tab"|"backtab") An up, down, paging or Tab key while a plain field is typing. The keys a single-line field has no edit for, handed to the config by name so a list drawn under the field can move its selection; the text and caret stay put and `on_change` does not fire. Left, Right, Home and End move the caret instead and never reach here (ADR-0236). Fires on key repeat too, so a held Down keeps moving. Without it these keys do nothing (ADR-0112).
