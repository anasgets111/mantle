...
| Notifications | Supervisor-owned bounded snapshot bridge, visible-ID gating, public authority claimed lazily on first `require`, never at boot | Snapshot bounds, staged-feed, presentation, and command tests | No raw D-Bus objects enter Lua. |
...
| MPRIS | Durable Supervisor-owned capability feed from the first implementation, with bounded rows, revisions, unavailable state, and validated commands | Startup, disconnect, stale generation, stale revision, reload-continuity, and shutdown tests | Supervisor discovers and caches player state independent of any generation; renderer only registers and receives snapshots. |
...
| Lock and auth | Separate process owns the session-lock protocol, greetd, and PAM; deferred until a caller exists. Polkit is a Supervisor-owned capability, not part of this process, deferred separately. | Disposable PAM account and compositor recovery check | Secrets (passwords) never enter Lua or renderer IPC; Polkit challenge metadata does. |
