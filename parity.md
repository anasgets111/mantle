...
| Layout and paint | Intrinsic rows and columns, bounded Unicode text, panel fills, and path-only PNG/JPEG assets | Nested layout, overflow, text, and cache-limit tests | femtovg on EGL is the locked target. CPU SHM is the bootstrap path behind the renderer trait. Add flex, SVG, or clipping only with a measured caller. |
