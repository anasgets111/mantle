...
| Drawing | No config-facing paths. Gradients fill a box, `mask` fades a subtree by a gradient or an image's alpha (ADR-0255), `shadow_*` and `content_blur` filter a subtree (ADR-0254), and `backdrop_blur` frosts what is under a box (ADR-0256), but no node can mask another. Add the smallest set a real component needs; SVG already covers static artwork |
