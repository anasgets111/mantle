...
has no size of its own, so give it `width` and `height` ([nodes](nodes.md#textfield)).

A field takes the keyboard only when both hold:

| Condition | Detail |
| :--- | :--- |
| The surface has keyboard focus | A `panel` needs `keyboard_interactivity = "OnDemand"` or `"Exclusive"` ([surfaces](surfaces.md)); a popup shown under the focused surface shares its keys |
...
| Target | Effect |
| :--- | :--- |
| `{ capability = "lock", action = "authenticate" }` | Checked by PAM; success unlocks the session. A `lock` surface needs exactly one reachable field with this target ([surfaces](surfaces.md)) |
| `{ capability = "polkit", action = "authenticate" }` | Answers the current polkit request ([capabilities](capabilities.md)) |
| `{ capability = "network", action = "connect" }` | The password for the network being joined |
...
| Show a tooltip on hover | [Tooltip](surfaces.md#tooltip), with `hover_rect` as the anchor |
...
[popup](surfaces.md#popup)'s `anchor_rect` wants. The click is a pointer release, so the popup may
...
**Drag to reorder.** Each row is an `on_drag` button in a keyed [`list`](nodes.md#list). The drag
...
See also: [nodes](nodes.md) (`button`, `textfield`, `list`), [surfaces](surfaces.md)
(`keyboard_interactivity`, popups, lock), [signals](signals.md) (state the handlers write),
[animation](animation.md) (press and hover motion), [capabilities](capabilities.md) (`lock`,
`polkit`, `network`).
