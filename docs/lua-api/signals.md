...
A capability (`mantle.<name>`, see [capabilities](capabilities.md)) is a signal too. It reads `nil`
...
| `cap:on_change(fn)`, `cap:invoke(action, ...)` | nothing | Capabilities only ([capabilities](capabilities.md)) |
...
`:set` checks what it stores. It refuses NaN, infinity, integers past ±(2^53−1) and strings over
64 KiB, and leaves tables unchecked. It checks no types: `state("x", 1):set({})` succeeds, and only
...
`on_change` ([capabilities](capabilities.md)) or a `timer` ([scripting](scripting.md)).
...
| Reload | Keeps its value across in-place reloads. Lost when the [Renderer](../../CONTEXT.md) process is replaced (a crash respawn or a shell restart) |
...
| Run a side effect when a capability changes | `on_change` ([capabilities](capabilities.md)), never a map |
...
| `layer`, `anchor`, `monitor`, `namespace`, `parent` or an `id` bound to a signal is refused | These are structural and take plain values only ([surfaces](surfaces.md)) |
