...
Mantle runs as two processes ([CONTEXT](../../CONTEXT.md)). The **Supervisor** is the `mantle`
...
[`process.run`](scripting.md#processrun) or [`persistent_table`](scripting.md#persistent_table). To
...
| :--- | :--- | :--- |
| `panel`, `window`, `popup`, `lock` | Surface constructors | [surfaces](surfaces.md) |
| `rect`, `row`, `column`, `text`, `icon`, `image`, `capture`, `shader`, `button`, `list`, `textfield` | Node constructors | [nodes](nodes.md) |
...
| `mantle` | Capabilities and renderer members | [capabilities](capabilities.md) |
| `process`, `session_process`, `persistent_table`, `timer`, `action`, `json`, `log`, `fuzzy`, `palette`, `fonts` | Scripting | [scripting](scripting.md) |
...
| Failure | Result |
| :--- | :--- |
| Startup evaluation raises | No scene. Surfaces paint nothing. The next successful reload brings the shell up |
| Reload evaluation raises (syntax error, runtime error, bad top-level return) | The previous scene stays on screen. [`mantle.rescue`](capabilities.md#renderer-members) becomes `{ is_rescue = true, error_log = "<the error>" }`. The error is logged |
...
`session_process` instead ([scripting](scripting.md)).
...
| A raise from `on_change`, `timer`, `process.run` callbacks or idle callbacks | A debug line: visible only with `mantle -vv` |
...
| 3 | `mantle -vv`, then `mantle log -f` | Errors raised in `on_change`, `timer`, `process.run` and idle callbacks |
...
[`session_process`](scripting.md#session_process). A top-level `process.run` starts a second copy
on every reload.
...
([renderer members](capabilities.md#renderer-members)).
