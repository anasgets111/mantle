...
Any property the node's kind accepts ([nodes](nodes.md#common-properties)) can be named; the value's
...
**Fade a tooltip popup in and out.** A [popup](surfaces.md#popup) closing skips the exit, so the
...
**Slide a notification out.** Removing an item from a keyed [`list`](nodes.md#list) makes it
...
See also: [signals](signals.md) (`pulse`, `delay`, `hover`), [nodes](nodes.md) (properties and
