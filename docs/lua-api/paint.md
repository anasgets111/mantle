...
Layout and per-kind properties are on [Nodes](nodes.md); easing any of these values is on
...
| `foreground` (`text`, `icon`, `textfield`), `z`, `scale`, `rotate`, `translate`, `origin`, `visible` | Also affect paint; documented on [Nodes](nodes.md#common-properties) |
...
A full-screen [panel](surfaces.md#panel) whose first child is a translucent scrim and whose
